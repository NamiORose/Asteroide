...
public class SlotCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<SharedSuggestionProvider> builder) {
        builder.executes(ctx -> {
            info(String.format("Selected slot %d.", mc.player.getInventory().selected+1));
        ...
        builder.then(argument("slot", IntegerArgumentType.integer()).executes(context -> {
            // YES; No restrictions
            mc.player.getInventory().setSelectedHotbarSlot(IntegerArgumentType.getInteger(context, "slot") - 1);
            return SINGLE_SUCCESS;
        }));
    }
}
