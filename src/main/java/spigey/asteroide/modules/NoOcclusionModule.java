...
public class NoOcclusionModule extends Module {
    ...
    private final Setting<Integer> lightLevel = sgGeneral.add(new IntSetting.Builder().name("Light Level").description("The light level to set.").defaultValue(1).sliderMin(0).sliderMax(15).onChanged((value) -> mc.levelRenderer.allChanged()).build());

    @Override public void onActivate(){ mc.levelRenderer.allChanged(); }
    @Override public void onDeactivate(){ mc.levelRenderer.allChanged(); }
